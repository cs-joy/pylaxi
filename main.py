...
class MainWidget(RelativeLayout):
    ...
    def update_horizontal_lines(self):
        ...
        xmin = self.get_line_x_from_index(start_index)  # central_line_x + offset * spacing + self.current_offset_x
        xmax = self.get_line_x_from_index(end_index)  # central_line_x - offset * spacing + self.current_offset_x
        # spacing_y = self.H_LINES_SPACING * self.height

        for i in range(0, self.H_NB_LINES):
            line_y = self.get_line_y_from_index(i)  # i * spacing_y - self.current_offset_y
    ...
    def update(self, dt):
        ...
        if not self.check_ship_collision() and not self.state_game_over:
            ...
            self.sound_music1.on_stop()
            self.sound_gameover_impact.on_play()
            Clock.schedule_once(self.play_game_over_voice_sound, 3)
            print("GAME OVER")

    def play_game_over_voice_sound(self, dt):
        if self.state_game_over:
            self.sound_gameover_voice.on_play()

    def on_menu_button_pressed(self):
        print("BUTTON")
        if self.state_game_over:
            self.sound_restart.on_play()
        else:
            self.sound_begin.on_play()
        self.sound_music1.on_play()
        self.reset_game()
